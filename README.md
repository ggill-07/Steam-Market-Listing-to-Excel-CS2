...
- `price_source` for no-wear snapshot items and manual overrides
...
- no-wear market items such as cases and stickers use a faster snapshot path that targets Steam commodity pricing instead of crawling every listing page

- for those commodity items, the tool now prefers Steam's live `itemordershistogram` lowest sell order, falls back to `priceoverview` if needed, and only falls back to render-page pricing after that

- each no-wear run appends one dated price snapshot row to the same file so case and sticker prices can be trended over time

- every exported row now includes `snapshot_date` and `snapshot_timestamp`, so the saved files can be filtered by fetch time later
...
- plain output filenames are saved into the project-level `exports/` folder by default, with auto-generated fetch files grouped into `exports/skins`, `exports/cases`, or `exports/stickers` when the item type is clear

- if `fetch` is run without `-o`, it derives a stable filename from the item name and places it in the matching subfolder, such as `skins/ak_47_redline_field_tested.xlsx`

- fetching the same wear-based item again updates that same file in place and reports how many listings were added, removed, or stayed the same
...
- choose one or more wear checkboxes for skins, or mark the query as `No wear / no float item` for cases, stickers, capsules, and similar items
...
- autocomplete suggestions are fetched from Steam's market search endpoint and then cached locally in pp_data/market_name_autocomplete_cache.json`r
- desktop autocomplete keeps StatTrak™ names intact instead of collapsing them into the non-StatTrak base item name

- no-wear items such as stickers and cases can be queued without choosing a wear

- desktop settings are stored locally in `app_data/desktop_app_settings.json`

- you can turn on a desktop option to store all case snapshots in one shared workbook at `exports/cases/all_cases.xlsx` instead of separate case files
...
- skin queries validate at add-time through the autocomplete/exact-market-name match, while bulk no-wear items still use the explicit queue validation flow
...
exports/skins/ak_47_redline_field_tested.xlsx
